...
const { User, Expense, Category, Notification, Approval } = require('./models');
  ...
  try {
    ...
    for (let i = 0; i < 120; i++) {
      ...
      const expense = {
        ...
        date: dateStr,
        status,
        merchant: template.merchants ? template.merchants[Math.floor(Math.random() * template.merchants.length)] : merchants[Math.floor(Math.random() * merchants.length)],
    ...
    await Notification.bulkCreate([
      { userId: emp1.id, title: 'Expense Approved', message: 'Your expense "IndiGo flight BLR–DEL" (₹7,850) has been approved by Vikram Patel', type: 'success' },
      ...
      { userId: mgrEng.id, title: 'New Expense Submitted', message: 'Arjun Reddy submitted "Flight to Bangalore" for ₹8,500', type: 'approval' },
      ...
      { userId: admin.id, title: 'Security Alert', message: 'User rahul.ex@company.com has been deactivated', type: 'error' },
    ]);
    ...
    console.log('\nDemo Accounts:');
    console.log('  Admin:    admin@company.com      / Admin123!   (Rajesh Sharma)');
    console.log('  Finance:  finance@company.com    / Finance123! (Priya Iyer)');
    console.log('  Manager:  manager@company.com    / Manager123! (Vikram Patel)');
    console.log('  Employee: john@company.com       / Employee123! (Arjun Reddy)');
    console.log('========================================\n');
